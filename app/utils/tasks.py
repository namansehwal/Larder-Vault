...
from app.models.model import User, Order_Detail, Order_Items, Product, Category
...
def daily_reminders():
    ...
    orders = Order_Detail.query.filter(Order_Detail.created_at.like(current_date_str + '%')).all()
...
def monthly_activity_report():
    ...
    for user in User.query.filter_by(role='user').all():
        user_orders = [order for order in Order_Detail.query.all() if order.user_id == user.id]
...
def store_manager_report():
    ...
    with open('data.csv', 'w', outfile='') as output_file:
        ...
        dict_writer.writerows(process_data(Product.query.all(), Category.query.all(), Order_Items.query.all()))
